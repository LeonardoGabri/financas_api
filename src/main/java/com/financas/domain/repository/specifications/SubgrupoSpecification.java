...
@AllArgsConstructor
@Slf4j
public class SubgrupoSpecification implements Specification<Subgrupo> {

    private SubgrupoFilter filtro;
    @Override
    public Predicate toPredicate(Root<Subgrupo> root, CriteriaQuery<?> criteriaQuery, CriteriaBuilder criteriaBuilder) {
        ...
        if(filtro.getGrupoId() != null){
            String grupoFiltro = "%" + filtro.getGrupoId().toUpperCase() + "%";
            log.debug("Filtrando por grupo: {}", grupoFiltro);
            predicates.add(criteriaBuilder.like(criteriaBuilder.upper(root.get("grupoId")), grupoFiltro));
        }
